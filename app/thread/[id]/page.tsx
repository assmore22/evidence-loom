...
export default function ThreadPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="mx-auto max-w-reading px-4 py-8 sm:px-5 md:px-8">
      ...
      {isError && <p className="mt-6 text-madder" role="alert">Could not read. <button className="underline" onClick={() => refetch()}>Retry</button></p>}
      ...
      {t && (
        <article className="mt-4">
          ...
          <dl className="border-t border-ink/15 mt-6 grid grid-cols-[6rem_1fr] gap-y-2 pt-4 text-sm">
            <dt className="label">Source</dt><dd className="break-all"><a className="text-indigo underline" href={t.sourceUrl} target="_blank" rel="noopener noreferrer">{t.sourceUrl}</a></dd>
