...
def _norm_review(raw) -> dict:
    ...
    if summary == "":
        summary = "Evidence Loom review outcome: " + outcome
    if rationale == "":
        rationale = summary
...
def _review_prompt(protocol: str, case: dict, evidence_text: str) -> str:
    return (
        "You are Evidence Loom V2, a GenLayer source-consensus contract for Evidence threads woven to public sources with review, contradiction reports, challenge and appeal lifecycle.\n" + SECURITY +
...
def _ruling_prompt(mode: str, case: dict, filing: dict, evidence_text: str) -> str:
    return (
        "You are Evidence Loom V2 resolving a " + mode + " filing.\n" + SECURITY +
        "\nCase JSON: " + json.dumps(case, sort_keys=True) +
...
class Weave(gl.Contract):
    ...
    def __init__(self) -> None:
        self.clock = 0
        self.protocol = "Evidence Loom cases require public sources, prompt-injection resistance, source/claim comparison, challenge rights, appeal rights and visible audit trails."
    ...
    def get_contract_stats(self) -> str:
        ...
        while i < len(self.cases):
            ...
            if case.get("outcome") == "supported":
                ...
                unclear += 1
            i += 1
        return json.dumps({"contract": "Evidence Loom V2", "cases": len(self.cases), "evidence": len(self.evidence),
    ...
    def get_frontend_bootstrap(self) -> str:
        return json.dumps({"contract": "Evidence Loom V2", "statuses": list(STATUSES), "outcomes": list(OUTCOMES),
                           "legacyNoun": "thread", "recentCases": json.loads(self.get_recent_cases(10)),
    ...
    def weave_thread(self, claim: str, source_url: str) -> int:
        fields = {}
        fields["legacyType"] = "thread"
        return int(self._create_case("thread", claim, claim, source_url, fields))

    @gl.public.write
    def assess_thread(self, item_id: int) -> None:
        case = self._load_case(str(item_id))
        if case.get("outcome") == "pending":
            self.review_with_genlayer(str(item_id))

    @gl.public.write
    def archive_thread(self, item_id: int) -> None:
    ...
    @gl.public.view
    def get_thread(self, item_id: int) -> dict:
        case = self._load_case(str(item_id))
        fields = case.get("fields", {})
        return {
            "weaver": case.get("actor", ""),
            "claim": case.get("title", ""),
        ...
        while i < len(self.cases):
            ...
            if case.get("status") != "ARCHIVED":
                ...
                if code == 1:
                    ...
                    pending += 1
            i += 1
        out = {"total": pos + neg + pending + uncertain, "upheld": pos, "frayed": neg, "pending": pending}
